...
class TD3:
    ...
    def critic_loss(self, state, action, next_state, reward, done):
        ...
        Q_next = torch.min(self.target_critic_1(next_state, next_action),
                           self.target_critic_2(next_state, next_action))
        target = reward + torch.logical_not(done) * GAMMA * Q_next
        loss1 = F.smooth_l1_loss(Q1, target)
        loss2 = F.smooth_l1_loss(Q2, target)

        return loss1, loss2

    def actor_loss(self, state):
        action = self.actor(state)
        loss = -self.critic_1(state, action)
    ...
    def update(self, transition):
        ...
        if len(self.replay_buffer) > BATCH_SIZE * 16:
            ...
            loss1, loss2 = self.critic_loss(state, action, next_state, reward, done)
            loss_actor = self.actor_loss(state)
